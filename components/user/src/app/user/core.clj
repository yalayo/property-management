   ...
   [:div.col-span-6
    ...
    [:input#Email.block.w-full.rounded-md.border-0.py-1.5.text-gray-900.shadow-sm.ring-1.ring-inset.ring-gray-300.placeholder:text-gray-400.focus:ring-2.focus:ring-inset.focus:ring-indigo-600.sm:text-sm.sm:leading-6
     {:type "email"
      ...
      :autocomplete "off"}]]
   [:div.col-span-6
    [:label.block.text-sm.font-medium.text-gray-700.dark:text-gray-200 {:for "password"} "Actual Password"]
    [:input#password.block.w-full.rounded-md.border-0.py-1.5.text-gray-900.shadow-sm.ring-1.ring-inset.ring-gray-300.placeholder:text-gray-400.focus:ring-2.focus:ring-inset.focus:ring-indigo-600.sm:text-sm.sm:leading-6
     {:type "password"
      :name "password"}]]
   [:div.col-span-6
    ...
    [:input#password.block.w-full.rounded-md.border-0.py-1.5.text-gray-900.shadow-sm.ring-1.ring-inset.ring-gray-300.placeholder:text-gray-400.focus:ring-2.focus:ring-inset.focus:ring-indigo-600.sm:text-sm.sm:leading-6
     {:type "password"
      :name "new-password"}]]
   [:div.col-span-6.sm:flex.sm:items-center.sm:gap-4
    [:button.inline-block.shrink-0.rounded-md.border.border-blue-600.bg-blue-600.px-12.py-3.text-sm.font-medium.text-white.transition.hover:bg-transparent.hover:text-blue-600.focus:outline-none.focus:ring.active:text-blue-500.dark:hover:bg-blue-700.dark:hover:text-white "Change password"]]])
...
(def change-password-handler
   {:name ::get
   :enter (fn [context]
            (assoc context :response (respond change-password-page)))})

(def post-change-password-handler
  {:name ::post
   :enter (fn [context]
            (let [params (-> context :request :params)
                  {:keys [email password new-password]} params
                  verify (verifyPassw {:psw password :email email})]
              (if (= (:status verify) true)
                (do
                  (db/change-password email new-password)
                  (assoc context :response {:status 200
                                            :headers {"HX-Redirect" "/flags"}}))
                (assoc context :response (-> (change-password-form {:error (:msg verify) :email email}) (ok))))))})
...
(def internal-routes
  #{["/sign-up"
    ...
    ["/change-password" 
     :post [(body-params/body-params) params/keyword-params post-change-password-handler]
